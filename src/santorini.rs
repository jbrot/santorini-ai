...
pub struct Board {
    grid: [u8; (BOARD_WIDTH.0 * BOARD_HEIGHT.0) as usize],
}

impl Board {
    fn new() -> Board {
        Board {
            grid: [0; (BOARD_WIDTH.0 * BOARD_HEIGHT.0) as usize]
        }
    }

    pub fn level_at(&self, loc: Point) -> CoordLevel {
        match self.grid[loc.offset() as usize] {
            0b0000 => CoordLevel::Ground,
            0b0001 => CoordLevel::One,
            0b0010 => CoordLevel::Two,
            0b0100 => CoordLevel::Three,
            0b1000 => CoordLevel::Capped,
            data => panic!("Invalid entry at {:?}: {}", loc, data),
        }
    }
    ...
    pub fn less_than_equals(&self, loc: Point, level: CoordLevel) -> bool {
        ...
        return self.grid[loc.offset() as usize] & mask == 0;
    }

    fn build(&mut self, loc: Point) {
        let data = &mut self.grid[loc.offset() as usize];
        let mask = match *data {
            0b0000 => 0b0001,
            0b0001 => 0b0011,
            0b0010 => 0b0110,
    ...
    fn cap(&mut self, loc: Point) {
        self.grid[loc.offset() as usize] = 0b1000;
    }
}
